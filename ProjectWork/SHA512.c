...
/*
Union operating similar to a struct.
Members operate on same memory.
Words and bytes take up same memory size.
1024 value is derivative of sha512.
*/
union Block 
{
    // 8 x 128 = 1024 - dealing with block as bytes.
    BYTE bytes[128];
    // 16 x 64 = 1024 - dealing with block as words.
    WORD words[32];
...
// Section 4.2.2
// Constant extracted from secure hash standard.
/*
The words represent the first 64 bits of the parts of cube root values,
 of the first eighty prime numbers.
Read left to right.
*/
const WORD K[] = 
{
   ...
   WORD H[] = 
   {
    ...
    else
    {
        ...
        else
        {
            ...
            printf("\n");
            // Calculate the SHA512 of in-file
            sha512(file, H);
